-- Add space_id to workflow and backfill to Personal space per user
ALTER TABLE "workflow" ADD COLUMN IF NOT EXISTS "space_id" uuid;
...
FROM (
  ...
  WHERE NOT EXISTS (
    ...
    WHERE sm.user_id = w.user_id AND s.status <> 'deleted'
  )
) users_without_space;

-- Add space memberships for the newly created Personal spaces
INSERT INTO space_member (id, space_id, user_id, role, created_at)
SELECT gen_random_uuid(), s.id, w.user_id, 'owner', NOW()
FROM workflow w
JOIN space s ON s.name = 'Personal' AND s.status = 'active'
WHERE NOT EXISTS (
  SELECT 1 FROM space_member sm
  WHERE sm.user_id = w.user_id AND sm.space_id = s.id
)
AND NOT EXISTS (
  SELECT 1 FROM space_member sm2
  JOIN space s2 ON s2.id = sm2.space_id
  WHERE sm2.user_id = w.user_id AND s2.status <> 'deleted'
);

-- Backfill workflow.space_id to user's first active space (or newly created Personal)
UPDATE workflow w SET space_id = (
...
CREATE INDEX IF NOT EXISTS workflow_space_id_idx ON workflow (space_id);
CREATE INDEX IF NOT EXISTS workflow_space_user_idx ON workflow (space_id, user_id);

-- Default visibility to public at DB level (if not already)
ALTER TABLE "workflow" ALTER COLUMN "visibility" SET DEFAULT 'public';
